...
namespace Step5.Core.Tests
{
  /// <summary>
  /// Tests for the <code>MessageSender</code>.
  /// 
  /// This is much better.  The two dependencies (INameRepository & IWriter) can be mocked, and the only unit of code being
  /// tested is the MessageSender.
  /// </summary>
  [TestFixture]
  ...
  {    
    ...
    public void SetUp()
    {
      // Strict means that if an expectation is not setup but it happens, the test will fail
